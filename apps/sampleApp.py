...
framework usage per IMPLEMENTATION.md specifications.
...
# Add parent directory to path for daemon module import
...
def create_sampleapp():
    ...
    Implements the routing pattern described in IMPLEMENTATION.md.
...
if __name__ == "__main__":
    ...
    Start with:
        python apps/sampleApp.py --server-port 8001
    
    Then test with:
        curl http://localhost:8001/
        curl http://localhost:8001/user
        curl -X POST http://localhost:8001/echo -H "Content-Type: application/json" -d '{"test":"data"}'
    """
    import argparse
    ...
    parser.add_argument(
        ...
        type=int,
        default=8001,
        help='Port number to bind the server. Default is 8001'
    )
